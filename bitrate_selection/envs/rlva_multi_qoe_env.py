...
class RLVAMultiQoEEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.chunk_remain = np.array([self.simulator.get_chunk_num()], dtype=np.float32)
...
def _test():
    ...
    rlva_env = RLVAMultiQoEEnv(config, 'Wu2017', '4G', [(1, 1, 1)], log_path, config.startup_download)
    random.seed(0)
    rlva_env.reset()
    rlva_env.simulator.end_chunk = 11
    rlva_env.simulator.net_trace.trace_len = 2
    print(isinstance(rlva_env, gym.Env))

    chunk, tile_id, over = 0, 0, False
    while chunk < 1000 and not over:
        action = random.randint(0, 4)
        state, reward, over, _ = rlva_env.step(action)
        print('Chunk:', chunk)
        print('Reward:', reward)
        print('Throughputs:', state['throughput'])
        print('Chunk sizes:', state['chunk_sizes'])
        print('Rebuffer:', state['rebuffer'])
        print('Last bitrates:', state['last_bitrates'])
        print('Predicted viewport:', state['pred_viewport'])
        print('Chunk remain:', state['chunk_remain'])
        chunk += 1
        if over:
            rlva_env.reset()
            break
