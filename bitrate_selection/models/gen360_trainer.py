...
from utils.gen360_utils import train_identifier, calculate_indentifier_reward
    ...
    def __next__(self) -> Union[None, Tuple[int, Dict[str, Any], Dict[str, Any]]]:
        ...
        with tqdm.tqdm(
            ...
            while t.n < t.total and not self.stop_fn_flag:
                ...
                # if self.args.use_identifier:
                #     # recompute reward
                #     samples, _ = self.train_collector.buffer.sample(0)
                #     for i in range(len(samples)):
                #         obs = samples.obs[i]
                #         action_one_hot = obs.action_one_hot
                #         qoe_reward = samples.rew[i]
                #         identifier_reward = calculate_indentifier_reward(self.identifier, obs, action_one_hot)
                #         samples.rew[i] = (1 - self.args.lamb) * qoe_reward + self.args.lamb * identifier_reward
                #         self.cnt += 1
                #         if self.cnt % self.observe_round == 0:
                #             print('Reward:', samples.rew[i], ' --- ', 'QoE Reward:', qoe_reward, ' --- ', 'Identifier Reward:', identifier_reward)
