...
class BasePolicy_Modified(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    @abstractmethod
    def learn(self, batch_ori: Batch, batch_sampled: Batch, **kwargs: Any) -> Dict[str, Any]:
    ...
    def update(self, sample_size: int, buffer: Optional[ReplayBuffer],
        ...
        self.updating = True
        batch_ori, batch_sampled = self.process_fn(batch, buffer, indices)
        # need to add other code to get the batch for sampled preference weight

        result = self.learn(batch_ori, batch_sampled, **kwargs)
    ...
    def compute_episodic_return(
        ...
        if v_s_ is None:
            ...
            v_s_ = v_s_ * BasePolicy_Modified.value_mask(buffer, indices)
    ...
    def compute_nstep_return(
        ...
        target_q_fn_sampled: Callable[[ReplayBuffer, np.ndarray], torch.Tensor],
        gamma: float = 0.99,
        n_step: int = 1,
        rew_norm: bool = False,
    ) -> Tuple[Batch, Batch]:
        # return the original batch and the sampled batch
        ...
        batch_sampled = copy.deepcopy(batch)
        ...
        terminal = indices[-1]
        with torch.no_grad():
            target_q_torch_ori = target_q_fn(buffer, terminal)  # (bsz, ?)
        target_q = to_numpy(target_q_torch_ori.reshape(bsz, -1))
        target_q = target_q * BasePolicy_Modified.value_mask(buffer, terminal).reshape(-1, 1)
        ...
        # sampled case
        with torch.no_grad():
            target_q_torch_sampled = target_q_fn_sampled(buffer, terminal)  # (bsz, ?)
        target_q_sampled = to_numpy(target_q_torch_sampled.reshape(bsz, -1))
        target_q_sampled = target_q_sampled * BasePolicy_Modified.value_mask(buffer, terminal).reshape(-1, 1)
        end_flag = buffer.done.copy()
        end_flag[buffer.unfinished_index()] = True
        target_q_sampled = _nstep_return(rew, end_flag, target_q_sampled, indices, gamma, n_step)

        batch_ori.returns = to_torch_as(target_q, target_q_torch_ori)
        batch_sampled.returns = to_torch_as(target_q_sampled, target_q_torch_sampled)
        if hasattr(batch_ori, "weight"):  # prio buffer update
            batch_ori.weight = to_torch_as(batch_ori.weight, target_q_torch_ori)
        if hasattr(batch_ori, "weight"):  # prio buffer update
            batch_sampled.weight = to_torch_as(batch_sampled.weight, target_q_torch_sampled)

        return batch_ori, batch_sampled
...
class DQNPolicy(BasePolicy_Modified):
    ...
    def _target_q_sampled(self, buffer: ReplayBuffer, indices: np.ndarray) -> torch.Tensor:
        batch_sampled = copy.deepcopy(buffer[indices])  # batch.obs_next: s_{t+n}
        # sampled_preference_weight = random.choice(QOE_TRAIN)
        # batch_sampled['obs']['prefernce_weight'] = sampled_preference_weight
        result, sampled_preference_weight = self.forward_sampled(batch_sampled, input="obs_next")
        if self._target:
            # target_Q = Q_old(s_, argmax(Q_new(s_, *)))
            target_q = self.forward_sampled(batch_sampled, preference_weight=sampled_preference_weight, model="model_old", input="obs_next")[0].logits
        else:
            target_q = result.logits
        if self._is_double:
            return target_q[np.arange(len(result.act)), result.act]
    ...
    def process_fn(
        ...
        """
        # return the original batch and the sampled batch
        batch_ori, batch_sampled = self.compute_nstep_return(
            batch, buffer, indices, self._target_q, self._target_q_sampled, self._gamma, self._n_step,
            self._rew_norm
        )
        return batch_ori, batch_sampled
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        # compute q value according to current preference weight
        logits, hidden = model(obs_next, state=state, info=batch.info)
        q = self.compute_q_value(logits, getattr(obs, "mask", None))
        if not hasattr(self, "max_action_num"):
            self.max_action_num = q.shape[1]
        act = to_numpy(q.max(dim=1)[1])
        tmp1 = Batch(logits=logits, act=act, state=hidden)
        tmp2 = batch
    ...
    def forward_sampled(
            self,
            batch: Batch,
            preference_weight: np.array = None,
            state: Optional[Union[dict, Batch, np.ndarray]] = None,
            model: str = "model",
            input: str = "obs",
            **kwargs: Any,
        ...
        model = getattr(self, model)
        batch_sampled = copy.deepcopy(batch)
        obs = batch_sampled[input]
        obs_next = obs.obs if hasattr(obs, "obs") else obs

        # compute q value according to current sampled weight
        if preference_weight is None:
            # sampled_preference_weight = random.choice(self.config.qoe_split['train'])
            sampled_preference_weight = obs_next['sample_weight']
        else:
            sampled_preference_weight = preference_weight
        obs_next['qoe_weight'] = sampled_preference_weight
        obs['qoe_weight'] = sampled_preference_weight
        # other features of obs_next will change because action may change due to the change of preference weight

        logits_sampled, hidden_sampled = model(obs_next, state=state, info=batch.info)
        q_sampled = self.compute_q_value(logits_sampled, getattr(obs, "mask", None))
        if not hasattr(self, "max_action_num"):
            self.max_action_num = q_sampled.shape[1]
        act_sampled = to_numpy(q_sampled.max(dim=1)[1])
        # return batch_sampled
        return Batch(logits=logits_sampled, act=act_sampled, state=hidden_sampled), sampled_preference_weight

    def learn(self, batch_ori: Batch, batch_sampled: Batch, **kwargs: Any) -> Dict[str, float]:
        if self._target and self._iter % self._freq == 0:
            self.sync_weight()
        self.optim.zero_grad()
        weight = batch_ori.pop("weight", 1.0)
        # get q value for current q value
        q = self(batch_ori).logits
        q = q[np.arange(len(q)), batch_ori.act]
        returns = to_torch_as(batch_ori.returns.flatten(), q)
        td_error = returns - q

        # code for debug
        # batch_debug = self.forward(batch)
        # q_sampled = batch_debug.logits
        # q_sampled = q_sampled[np.arange(len(q_sampled)), batch_debug.act]
        # tmp, _ = torch.max(batch_debug.logits, dim=1)
        # returns_sampled = to_torch_as(tmp.flatten(), q_sampled)
        # td_error_debug = returns_sampled - q
        # print('td_error_debug', td_error_debug)

        # get q value for sampled q value

        q_sampled = self(batch_sampled).logits
        q_sampled = q_sampled[np.arange(len(q_sampled)), batch_sampled.act]
        returns_sampled = to_torch_as(batch_sampled.returns.flatten(), q_sampled)
        td_error_sampled = returns_sampled - q_sampled
        # print('td error is:', td_error.pow(2))
        # print('td error sampled is:', td_error_sampled.pow(2))
        td_error_combine = 0.5 * (td_error.pow(2) + td_error_sampled.pow(2))

        loss = (td_error_combine * weight).mean()
        batch_ori.weight = td_error  # prio-buffer
        batch_sampled.weight = td_error  # prio-buffer
        loss.backward()
        self.optim.step()
        self._iter += 1
