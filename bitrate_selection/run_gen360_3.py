    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(sample_count), desc='Testing: '):
            ...
            while not done:
                ...
                if video == 2 and user == 17 and trace == 31:
...
def case_study(args, config, policy, qoe_weights, identifier, models_dir, results_dir):
    ...
    target_video, target_user, target_trace = 2, 45, 27
    qoe_weight1 = np.array([1.0, 7.0, 1.0], dtype=np.float32)
    qoe_weight2 = np.array([7.0, 1.0, 1.0], dtype=np.float32)
    qoe, qoe1, qoe2, qoe3 = [], [], [], []
    with torch.no_grad():
        sample_count = test_env.sample_count()
        for i in tqdm(range(sample_count), desc='Testing: '):
            ...
            while not done:
                ...
                chunk += 1
            print()
    case_study_path = os.path.join(results_dir, f'results_case_study_uid_{args.use_identifier}_v_{target_video}_u_{target_user}_t{target_trace}_fqoe_{tuple(map(int, qoe_weight1.tolist()))}_sqoe_{tuple(map(int, qoe_weight2.tolist()))}.csv')
...
def run(args, config):
    ...
    if args.test:
        ...
        # case_study(args, config, policy, qoe_weights, identifier, models_dir, results_dir)
...
if __name__ == '__main__':
    ...
    # No BC
    # python run_gen360_3.py --epoch 1000 --step-per-epoch 5000 --step-per-collect 2500 --lr 0.0005 --batch-size 512 --train --train-dataset Wu2017 --test --test-dataset Wu2017 --qoe-test-ids 0 1 2 3 --test-on-seen --lamb 0.5 --train-identifier --identifier-epoch 1000  --device cuda:0 --gamma 0.95 --ent-coef 0.02 --seed 666 --use-identifier
    # test (only)
