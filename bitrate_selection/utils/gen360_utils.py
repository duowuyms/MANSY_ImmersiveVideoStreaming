...
def train_identifier(identifier, identifier_optim, tracjetory, update_round=2):
    ...
    for _ in range(update_round):
        # action_one_hot = samples['obs']['action_one_hot']
        # pred_qoe_weight = identifier(samples['obs'], action_one_hot)
        # gt_qoe_weight = torch.from_numpy(samples['obs']['qoe_weight']).to(identifier.device)
...
def calculate_indentifier_reward(identifier, state, action_one_hot):
    ...
    # identifier_reward = -mse
    identifier_reward = 1 - mse
    return identifier_reward.cpu().numpy()
    # mse = mse.cpu().numpy()
    # identifier_reward = - np.log(mse + 1e-4)
    # return identifier_reward
...
def behavior_cloning_pretraining(args, policy, identifier, policy_optim, identifier_optim, train_demos, valid_demos, 
    ...
    for i in range(max_steps):
        ...
        #     batch = samples[j:min(len(samples), j + bs)]
        #     results = policy(batch)
        #     logits, act, dist = results.logits, results.act, results.dist
        #     loss = criterion(logits, torch.tensor(batch['act']).to(args.device)) - 0.1 * dist.entropy().mean()
        #     policy_optim.zero_grad()
        #     loss.backward()
        #     policy_optim.step()
        #     print(f'BC (Training): loss={loss.item()} ({i + 1}/{max_steps}) - batch {j}')
