...
from envs.rlva_env import RLVAEnv
from models.rlva import FeatureNet, Actor, Critic
...
def train(args, config, policy, qoe_weights, models_dir, file_prefix):
    ...
    env = RLVAEnv(config, args.train_dataset, args.network_dataset, qoe_weights, valid_log_path, config.startup_download,
    ...
    train_env = SubprocVectorEnv(
        [lambda: RLVAEnv(config, args.train_dataset, args.network_dataset, qoe_weights, train_log_path, config.startup_download,
    ...
    valid_env = DummyVectorEnv(
        [lambda: RLVAEnv(config, args.train_dataset, args.network_dataset, qoe_weights, valid_log_path, config.startup_download,
...
def test(args, config, policy, qoe_weights, models_dir, results_dir, file_prefix):
    ...
    test_env = RLVAEnv(config, args.test_dataset, args.network_dataset, qoe_weights, test_log_path, config.startup_download,
...
def run(args, config):
    ...
    actor = Actor(feature_net, feature_dim=6 * 128, action_space=config.action_space, device=args.device)
    critic = Critic(feature_net, feature_dim=6 * 128, device=args.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='rlva')
    ...
    # experiment special
    parser.add_argument('--model', type=str, default='rlva')
